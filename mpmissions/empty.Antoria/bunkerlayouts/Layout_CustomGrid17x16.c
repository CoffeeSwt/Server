class Layout_CustomGrid17x16 : BaseLayout
{
	override void Spawn(vector origin)
	{
		...
		// 2) Halls
		FP_GridFillRect( grid,	4,	6,	2,	6,	FP_HALL );
		FP_GridFillRect( grid,	6,	10,	2,	7,	FP_HALL );
		FP_GridFillRect( grid,	8,	15,	3,	2,	FP_HALL );
		FP_GridFillRect( grid,	6,	7,	5,	2,	FP_HALL );
		FP_GridFillRect( grid,	11,	7,	2,	3,	FP_HALL );
		FP_GridFillRect( grid,	11,	10,	2,	3,	FP_HALL );

		// 3) Rooms
		FP_GridFillRect( grid,	1,	12,	5,	5,	FP_ROOM );
